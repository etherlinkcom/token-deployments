...
async function main() {
  ...
  const isPeer = await WXTZ.isPeer(endpointIds[targetNetworkName], customFormatBytes32String(WXTZDeployed[targetNetworkName]))
  console.log(`${networkName} <> ${targetNetworkName} is peer: ${isPeer}\n`);
}
