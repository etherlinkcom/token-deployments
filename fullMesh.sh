...
for (( i=0; i<${#networks[@]}; i++ )); do
  for (( j=i+1; j<${#networks[@]}; j++ )); do
    ...
    echo "Running: targetNetworkName=$network_A npx hardhat run --network $network_B scripts/setPeer.ts"
    targetNetworkName=$network_A npx hardhat run --network $network_B scripts/setPeer.ts

    # Run the command with B as target and A as source
    echo "Running: targetNetworkName=$network_B npx hardhat run --network $network_A scripts/setPeer.ts"
    targetNetworkName=$network_B npx hardhat run --network $network_A scripts/setPeer.ts
  done
done
